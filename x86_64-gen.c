...
static int gfunc_arg_size(CType *type) {
    int align;
    return size_align_of_type(type->t, &align);
}
    ...
    for(i = 0; i < nb_args; i++) {
        ...
        if (!using_regs(size)) {
            ...
            if (is_sse_float(vtop->type.t)) {
                if (arg >= REGN) {
                    gen_ldr_reg(TREG_XMM0);
                    /* movq %xmm0, j*8(%rsp) */
                    gen_offs_sp(0xd60f66, 0x100, arg*8);
                } else {
                    /* Load directly to xmmN register */
                    gen_ldr_reg(TREG_XMM0+arg);
                    d = arg_prepare_reg(arg);
                    /* mov %xmmN, %rxx */
                    o(0x66);
                    orex(1,d,0, 0x7e0f);
...
{
    int v, saved_ind,btype,size,align;
